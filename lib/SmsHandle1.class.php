...
class SmsHandle {
	...
	public function charge($devicesn,$username,$smsfree,$wxres,$boxId,$station_limit,$user_limit,$content){
		...
		// 格口收费时短信是否还收费
		if ($smsfree == 1 && $wxres != 2 && $station_limit['smssend']==1 && $user_limit['smssend']==1) {
		...
		if($befor_fund > $after_fund){
			//扣除费用记录
			$sqlstr = "INSERT INTO  deeyee.consumingrecords (`username` ,`beforsend` ,`aftersend` ,`msm_num` ,`operation` ,`time` ) 
		...
		{
			//格口费用记录
			$sqlstr = "INSERT INTO  deeyee.consumingrecords (`username` ,`beforsend` ,`aftersend` ,`msm_num` ,`operation` ,`time`,`type` ) 
		VALUES ('$username',  '$after_fund',  '$after_fund1',  '1',  '$devicesn',  '$sendtime','$boxsize')";
			mysql_query($sqlstr, $db);
		}
