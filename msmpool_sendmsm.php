...
function msmpool_sendmsm() {
    ...
    $limit_n = 100;
    //1)标记出要操作的记录
    mysql_query("UPDATE `msmpool` SET send_n=5  where  status>1 or wxstatus>1  order by id asc  LIMIT $limit_n", $db1); //100
    ...
    {
        mysql_query("UPDATE `msmpool` SET `send_n` = '0'", $db1);
        return;
    }
    for ($i = 0;$i < $num;$i++) {
        ...
        if($smssendflg==1&&$wxsendflg==1){
            ...
            if($status==2 || $wxstatus==2){//如果短信或微信有一个发送成功，则同步状态为成功
                tongbu($msm_sn,$devicesn,'2',$stationid,$db2,$db4); 
            }elseif($status==3 && $wxstatus==3){//如果短信和微信都发送失败，则同步状态为失败
                tongbu($msm_sn,$devicesn,'3',$stationid,$db2,$db4); 
            }
            // 如果不是短信和微信状态都回来了，那不能拷贝到sendmsm表
            if(!($status>1 && $wxstatus>1)){
                mysql_query("UPDATE `msmpool` SET `send_n` = '0' where msm_sn='$msm_sn'", $db1);
            }
        }elseif($smssendflg==1&&$wxsendflg==0){
            // 只发短信
            if($status>1){
                // 智能柜和pad状态同步
                tongbu($msm_sn,$devicesn,$status,$stationid,$db2,$db4); 
            }
        }elseif($smssendflg==0&&$wxsendflg==1){
            // 只发微信
            if($wxstatus>1){
                // 智能柜和pad状态同步
                tongbu($msm_sn,$devicesn,$wxstatus,$stationid,$db2,$db4); 
            }
        }
    }
    //将标出的记录拷贝到sendmsm表中
...
// 智能柜和pad状态同步
function tongbu($msm_sn,$devicesn,$status,$stationid,$db2,$db4) {
  // 如果是用智能柜发送的短信
  if($devicesn){
    $result1 = mysql_query("SELECT id FROM  smtbx_order  where  devicesn=$devicesn and  ordersn='$msm_sn'", $db2);
...
function tosendmsm($db){
    mysql_query("SET AUTOCOMMIT=0"); //开始事务处理
     $sqlstr = "
    INSERT INTO ksendmsm ( `username` ,`rcvnumber` ,`sendtime` ,`huohao` ,`stationid` ,`frequency` ,`status` ,`content`,`m_operator`,`msm_sn`,`companyflg`,`devicesn`,`wxcontent`,`smsfree`,`smssendop`,`smssendflg`,`wxsendop`,`wxsendflg`,`wxstatus`,`boxid`,`password`,`expressno`,`mode`,`manager` )   SELECT   `username` ,`rcvnumber` ,`sendtime` ,`huohao` ,`stationid` ,`frequency` ,`status` ,`content`,`m_operator`,`msm_sn` ,`companyflg`,`devicesn`,`wxcontent`,`smsfree`,`smssendop`,`smssendflg`,`wxsendop`,`wxsendflg`,`wxstatus`,`boxid`,`password`,`expressno`,`mode`,`manager`  FROM   kmsmsend.msmpool    WHERE    send_n=5";
     $result1 = mysql_query($sqlstr, $db);
     //删除已经标出的记录
     $sqlstr = "DELETE FROM  kmsmsend.msmpool  WHERE    send_n=5";
